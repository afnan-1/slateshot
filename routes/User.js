    ...
    if (user) {
      ...
      User.findOne({ email }, (err, user) => {
        ...
        if (user) {
          ...
          if (!fs.existsSync(`./client/public/uploads/${email}`)) {
            fs.mkdir(`./client/public/uploads/${email}`, function (err) {
              if (err) {
                console.log(err);
              } else {
            ...
            newUser.save((err) => {
              if (err)
                ...
                res
                  ...
                  .json({
                    message: {
                      msgbody: "Account has succesfully created",
                      msgError: false,
                      emailError: false,
                    },
                  });
...
userRouter.post(
  ...
  (req, res) => {
    if (req.isAuthenticated()) {
      ...
      res
        .status(200)
        .json({ isAuthenticated: true, user: req.user, token: token });
    }
  }
);
userRouter.post("/logingoogle", (req, res) => {
  ...
  User.findOne({ email }, (err, user) => {
    if (err) {
      res
        .status(500)
        .json({ message: { msgbody: "error has occured", msgError: false } });
    }
    if (user) {
      res
        .status(200)
      ...
      res
        .status(500)
        .json({ message: { msgbody: "successfull", msgError: true } });
    }
  });
});
